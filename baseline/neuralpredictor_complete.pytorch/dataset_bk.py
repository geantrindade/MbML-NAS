...
class Nb101Dataset(Dataset):
    ...
    def __init__(self, split=None, exclude_split=[], epoch_set=0, debug=False):
        self.hash2id = dict()
...
class DatasetPred(Dataset):
    ...
    def __init__(self, classifier_indexes, exclude_split, epoch_set=0, debug=False):
        self.hash2id = dict()
        self.EPOCH_SET = epoch_set
        with h5py.File("data/nasbench.hdf5", mode="r") as f:
            for i, h in enumerate(f["hash"][()]):
        ...
        if classifier_indexes == 'all':
            sample_set = np.array(range(len(self.hash2id)))
        else:
            sample_set = classifier_indexes
