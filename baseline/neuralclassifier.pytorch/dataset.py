...
class Nb101Dataset(Dataset):
    ...
    def _is_class_high_threshold(self, acc):
        return acc >= self.ACC_THRESHOLD #0.91
    ...
    def __getitem__(self, index):
        ...
        for seed_test_avg in [0, 1, 2]:
            _, test_acc = self.metrics[index, self.EPOCH_SET, seed_test_avg, -1, 2:]
            test_avg += test_acc
            
        test_acc = test_avg / 3

        if self._is_class_high_threshold(val_acc):
            val_acc = 1
        else:
            val_acc = 0
        
        if self._is_class_high_threshold(test_acc):
            test_acc = 1
        else:
            test_acc = 0
