...
def accuracy_bin(target, predict):
    predict = [decision(x) for x in predict]
    accu = acc(target, predict)
    accu = str.format('%0.4f' % accu)
    return accu
...
def main(base_dir, id_run, n_e, seed, train_size, epoch_set):

    path_logs = str.format('%s/%s/' % (base_dir, id_run))
    ...
    vis_log = VisLog(id_run, path_logs, 100, 'tt')

    args = parser.parse_args()

    dataset_epochs_dict = ['4', '12', '36', '108']
    ...
    args.seed = seed
    args.trainset_epochs = dataset_epochs_dict[epoch_set]
    ...
    dataset = Nb101Dataset(split=train_split_seed, epoch_set=epoch_set, seed=seed)
    dataset_test = Nb101Dataset(split=args.eval_split, exclude_split=train_split_seed, epoch_set=epoch_set, seed=seed)
    ...
    for epoch in pbar:
        ...
        for step, batch in enumerate(data_loader):
            ...
            predict = net(batch)#.view((target.size()[0], -1))
        ...
        acc = accuracy_bin(target_run, pred_run)
    ...
    with torch.no_grad():
        ...
        for step, batch in pbar_t:
            ...
            acc = accuracy_bin(target, predict)
...
if __name__ == "__main__":
    ...
    for epoch_set, r_n in enumerate(runs_n):
        base_dir = str.format('runs/runs%s' % r_n)
        try:
            os.mkdir(base_dir)
        ...
        for n_e in epochs:
            for seed in seeds:
                for train_size in train_sizes:
                    ...
                    main(base_dir, run_dir_name, n_e, seed, train_size, epoch_set)
                    print('\n=======================================\n')
